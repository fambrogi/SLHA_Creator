...
def SLHA_Write(SMS, M,D,I, Out_Dir, t = ''):
        ...
        slha = ''
        if t : slha = open(Out_Dir+'/'+SMS+'_' +str(M)+'_'+str(M+4.9)+'_'+str(I)+'_'+str(D)+'_'+str(M)+'_'+str(I)+'_'+str(D)+'.slha' , 'w')
        else  : slha = open(Out_Dir+'/'+SMS+'_'+str(M)+'_'+str(M+4.9)+'_'+str(D)+'_'+str(M)+'_'+str(M+4.9)+'_'+str(D)+'.slha' , 'w')
        lines_in = temp.readlines()
        for line in lines_in:
            line = line.replace('MOTHER' , str(M))
            line = line.replace('NEUTRALINO2' , str(M2))
